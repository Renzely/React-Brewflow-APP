    ...
    <ScrollView style={styles.safeArea}>
      ...
      <TouchableOpacity
        ...
        style={[
          styles.customButton,
          {
            backgroundColor: attendanceData.hasTimedIn ? "#ccc" : "#2c1c5c",
          },
        ]}
      >
        {isLoadingTimeIn ? (
      ...
      <TouchableOpacity
        style={[styles.button, { backgroundColor: "#5134ab", marginTop: 20 }]}
...
const InventoryCard: React.FC<{ item: InventoryItem }> = ({ item }) => {
  ...
  // 🔴 New mapping to link SKU to SKUCode
  const skuCodeMapping: Record<
    VersionKey,
    Record<string, string>
  > = {} as Record<VersionKey, Record<string, string>>;

  version.forEach((ver) => {
    const carriedSKUs = versions[ver]?.Carried || [];

    skuValues.ending[ver] = Object.fromEntries(
      carriedSKUs.map((sku) => [sku.sku, sku.endingPCS.toString()])
    );

    skuValues.expiry[ver] = carriedSKUs.reduce((acc, sku) => {
      acc[sku.sku] = Number(sku.expiryMonths); // ensure number
      return acc;
    }, {} as Record<string, number>);

    skuValues.quantity[ver] = Object.fromEntries(
      carriedSKUs.map((sku) => [sku.sku, sku.expiryQty.toString()])
    );

    // 🔴 Add SKU-to-SKUCode mapping
    skuCodeMapping[ver] = Object.fromEntries(
      carriedSKUs.map((sku) => [sku.sku, sku.skuCode])
    );
    ...
    const carriedCodes = new Set(
      (versions[ver]?.Carried || []).map((s) => s.sku)
    );
    const notCarriedCodes = new Set(
      (versions[ver]?.["Not Carried"] || []).map((s) => s.sku)
    );

    availability[ver] = Object.fromEntries(
      allSKUs.map((sku) => [
        sku.sku,
        carriedCodes.has(sku.sku)
          ? "Carried"
          : notCarriedCodes.has(sku.sku)
          ? "Not Carried"
          : "Delisted",
      ])
    );
  ...
  const renderSkuDetails = (sku: any, status: string) => (
    <View key={sku.skuCode} style={{ marginBottom: 10 }}>
      ...
      <Text style={[styles.itemText]}>SKU Code: {sku.skuCode}</Text>
  ...
  const handleGoToNextWeek = () => {
    try {
      ...
      console.log("Next Week Data:", JSON.stringify(nextWeekData, null, 2));
      console.log("SKU Code Mapping:", JSON.stringify(skuCodeMapping, null, 2));
      ...
      router.push({
        ...
        params: {
          ...
          previousWeekId: item._id,
          skuCodeMapping: JSON.stringify(skuCodeMapping),
        },
      });
    } catch (error) {
    ...
    >
      ...
      >
        ...
        {!item.isOffline && (
          <TouchableOpacity
            style={{
              backgroundColor: isLocked ? "#e0e0e0" : "#2c1c5c",
              padding: 8,
              borderRadius: 20,
            }}
            onPress={() => {
